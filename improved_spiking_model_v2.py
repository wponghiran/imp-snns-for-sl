...
class ImprovedSpikingModelV2(nn.Module):
    def __init__(self, options, inp_dim):
        super(ImprovedSpikingModelV2, self).__init__()
        # Reading parameters
        ...
        self.linear_us = nn.ModuleList([
            nn.Linear(self.hidden_size, 2 * self.hidden_size, bias=False) for i in range(self.num_layers)])
        if self.use_bn:
        ...
        for i in range(self.num_layers):
            ...
            nn.init.orthogonal_(self.linear_us[i].weight, gain=nn.init.calculate_gain('relu'))
        ...
        for i in range(self.num_layers):
            ...
            if self.do_edat:
                ...
                for t in range(seq_len):
                    u = self.linear_us[i](sparse_h)
                    uz, uc = u.chunk(2, -1)

                    z = torch.sigmoid(wzs[t] + uz)
                    c = nn.functional.relu(wcs[t] + uc)
                    if self.training:
