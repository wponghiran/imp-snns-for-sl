...
def create_configs(config):
    ...
    for arch in arch_lst:
        ...
        threshold[arch] = [0 for i in range(N_ep)]
        target_threshold[arch] = expand_str_ep(config[arch]["threshold"], "int", N_ep, "|", "*")
...
def change_lr_th_cfg(cfg_file, lr, th, ep):

    config = configparser.ConfigParser()
    config.read(cfg_file)

    for lr_arch in lr.keys():
        config.set(lr_arch, 'threshold', str(th[lr_arch][ep]))
