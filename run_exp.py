...
print('----------------------------------------------')
print('      Running with fixed threshold')
print('----------------------------------------------')
...
for arch in arch_lst:
    ...
    threshold[arch] = expand_str_ep(config[arch]["threshold"], "int", N_ep, "|", "*")
    ...
    min_lr[arch] = float(config[arch]["min_lr"])
    patience[arch] = int(config[arch]["patience"])
    ...
    for tr_data in tr_data_lst:
        ...
        for ck in range(N_ck_tr):
            ...
            if do_validation_after_chunk(ck, N_ck_tr, config):
                ...
                if not _is_first_validation(ep,ck, N_ck_tr, config):
                    # # Use validation loss as a metric
                    # metric_mean = np.mean(np.asarray(list(valid_peformance_dict.values()))[:, 0])
                    # metric_mean_prev = np.mean(np.asarray(list(valid_peformance_dict_prev.values()))[:, 0])
                    # Use error as metric
                    metric_mean = np.mean(np.asarray(list(valid_peformance_dict.values()))[:, 1])
                    metric_mean_prev = np.mean(np.asarray(list(valid_peformance_dict_prev.values()))[:, 1])
                     
                    if metric_mean < metric_mean_best:
                        for pt_arch in pt_files.keys():
                            if os.path.exists(model_files[pt_arch]):
                                copyfile(model_files[pt_arch], out_folder + "/exp_files/best_" + pt_arch + ".pkl")
                        metric_mean_best = metric_mean

                    for lr_arch in lr.keys():
                        if ep < N_ep - 1 and auto_lr_annealing[lr_arch]:
                            if ((metric_mean_prev - metric_mean) / metric_mean) < improvement_threshold[lr_arch]:
                                num_bad_epochs += 1
                            else:
                                num_bad_epochs = 0
                            if num_bad_epochs > patience[lr_arch]:
                                num_bad_epochs = 0
                                new_lr_value = max(float(lr[lr_arch][ep]) * halving_factor[lr_arch], float(min_lr[lr_arch]))
...
res_file.write("Best error rate is {:.3f}%\n".format(metric_mean_best))
res_file.close()
...
# for forward_mode in ['best', 'last']:
for forward_mode in ['best']:
    # --------FORWARD--------#
    for forward_data in forward_data_lst:
        ...
        for ck in range(N_ck_forward):
            ...
            if not is_production:
                print("Testing %s chunk (%s) = %i / %i" % (forward_data, forward_mode, ck + 1, N_ck_forward))
            else:
            ...
            info_file = (
                out_folder
                + "/exp_files/forward_{}_".format(forward_mode)
                + forward_data
                + ".info"
            )
            config_chunk_file = (
            ...
            # update learning rate in the cfg file (if needed)
            change_lr_cfg(config_chunk_file, lr, N_ep-1)
    ...
    for data in forward_data_lst:
        for k in range(len(forward_outs)):
            if forward_dec_outs[k]:
                ...
                print("Decoding %s (%s) output %s" % (data, forward_mode, forward_outs[k]))
    
                info_file = out_folder + "/exp_files/decoding_{}_".format(forward_mode) + data + "_" + forward_outs[k] + ".info"
    
                # create decode config file
                config_dec_file = out_folder + "/decoding_{}_".format(forward_mode) + data + "_" + forward_outs[k] + ".conf"
                ...
                files_dec = out_folder + "/exp_files/forward_{}_".format(forward_mode) + data + '_' + forward_outs[k] + "_to_decode.ark"
                out_dec_folder = out_folder + "/decode_{}_".format(forward_mode) + data + "_" + forward_outs[k]
