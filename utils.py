...
def create_configs(config):
    ...
    model_files_last = {}
    for arch in pt_files.keys():
        model_files_best[arch] = out_folder + "/exp_files/best_" + arch + ".pkl"
        model_files_last[arch] = out_folder + "/exp_files/final_" + arch + ".pkl"
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                if do_validation_after_chunk(ck, N_ck_tr, config):
                    ...
                    if strtobool(config["batches"]["increase_seq_length_train"]):
                        if len(max_seq_length_train.split(",")) == 1:
                            max_seq_length_train_curr = int(max_seq_length_train_curr * float(
    ...
    for forward_data in forward_data_lst:
        ...
        for ck in range(N_ck_forward):
            ...
            for forward_mode, assoc_model_files in [('forward_last_', model_files_last), ('forward_best_', model_files_best)]:
                # path of the list of features for this chunk
                lst_file = (
                    out_folder
                    + "/exp_files/" + forward_mode
                    + forward_data
                    + "_*.lst"
                )
...
def write_cfg_chunk(
    ...
    for arch in threshold.keys():
        config_chunk[arch]["threshold"] = str(threshold[arch][ep])
...
def forward_model(
    ...
    for line in model:
        ...
        if operation == "compute":
            ...
            if len(inp_out_dict[inp2]) > 1:  # if it is an input feature
                ...
                if to_do == "forward":
                    outs_dict[out_name], stat_zeroact_inps[out_name], stat_act_inps[out_name], stat_zeroact_rcrs[out_name], stat_act_rcrs[out_name], stat_ops[out_name] = nns[inp1](inp_dnn)
                else:
    ...
    if to_do == "forward":
        return outs_dict, stat_zeroact_inps, stat_act_inps, stat_zeroact_rcrs, stat_act_rcrs, stat_ops
    else:
        return outs_dict


def dump_epoch_results(
    res_file_path, ep, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, valid_data_lst, valid_peformance_dict, lr, threshold, N_ep
):
    ...
    for arch in threshold.keys():
        res_file.write("th_%s=%s " % (arch, threshold[arch][ep]))
        print("Threshold on %s = %s " % (arch, threshold[arch][ep]))
    print("-----")
...
def change_lr_cfg(cfg_file, lr, ep):
    ...
    for lr_arch in lr.keys():
        config.set(lr_arch, 'arch_lr', str(lr[lr_arch][ep]))
