...
Modified Pytorch-Kaldi Toolkit to simulate and train spiking neural networks with improved inherence recurrent dynamics.

For details on 
- how threshold is applied and low-precision output is generated during training and inference, see fake_spike_quantization.py
- how the traditional spiking model works, see traditional_spiking_model.py
- how the proposed spiking model with improved inherence recurrent dynamics works, see improved_spiking_model_vXX.py
- how training hyper-parameters are set, see ./cfg/

Execute training and inference by running `python run_exp.py $cfg_file` where `$cfg_file` is configuration file in cfg directory

Inference statictis will be generated at the end of training including
...
The following text is in the README file of the origin project.
