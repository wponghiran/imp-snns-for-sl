...
#         num_bits = 4
#         threshold = 1
#         nns[net].module.fsq_x.update_num_bits_and_threshold(num_bits, threshold)
#         for each in nns[net].module.fsq_hs:
#             each.update_num_bits_and_threshold(num_bits, threshold)
#     
#         print('--------- after ----------')
#         print(nns)
    ...
    for i in range(N_batches):
        ...
        if to_do == "train":
            ...
            # Clip decay rate to (0,1)
            for net in nns.keys():
                for name, param in nns[net].named_parameters():
                    if 'taus' in name:
                        w = param.data
                        w = w.clamp(0, 1)
            ...
            with torch.no_grad():  # Forward input without autograd graph (save memory)
                if to_do == "forward":
                    outs_dict, stat_zeroact_inps, stat_act_inps, stat_zeroact_rcrs, stat_act_rcrs, stat_ops = forward_model(
                        fea_dict,
                        lab_dict,
                        arch_dict,
                        model,
                    ...
                    for key in stat_zeroact_inps.keys():
                        num_zeroact_inps += int(torch.sum(stat_zeroact_inps[key]))
                        num_act_inps += int(torch.sum(stat_act_inps[key]))
                        num_zeroact_rcrs += int(torch.sum(stat_zeroact_rcrs[key]))
                        num_act_rcrs += int(torch.sum(stat_act_rcrs[key]))
                        num_ops += int(torch.sum(stat_ops[key]))
                else:
                    outs_dict = forward_model(
                        fea_dict,
                        lab_dict,
    ...
    if to_do == "forward":
        for out_name in forward_outs:
            post_file[out_name].close()
        print("num_zeroact_inps={}".format(num_zeroact_inps))
        print("num_act_inps={}".format(num_act_inps))
        print("num_zeroact_rcrs={}".format(num_zeroact_rcrs))
        print("num_act_rcrs={}".format(num_act_rcrs))
        print("avg_ops={}".format(num_ops/N_batches))
    ...
    with open(info_file, "w") as text_file:
        ...
        if to_do != "forward":
            ...
            text_file.write("num_zeroact_inps={}\n".format(num_zeroact_inps))
            text_file.write("num_act_inps={}\n".format(num_act_inps))
            text_file.write("num_zeroact_rcrs={}\n".format(num_zeroact_rcrs))
            text_file.write("num_act_rcrs={}\n".format(num_act_rcrs))
            text_file.write("avg_ops={}\n".format(num_ops/N_batches))
        text_file.write("elapsed_time_chunk=%f\n" % elapsed_time_chunk)
