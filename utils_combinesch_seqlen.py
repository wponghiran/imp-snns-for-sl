...
def create_configs(config):
    ...
    for arch in arch_lst:
        ...
        threshold_arr = expand_str_ep(config[arch]["threshold"], "float", N_ep, "|", "*")
        threshold[arch] = threshold_arr
...
def dump_epoch_results(
    res_file_path, ep, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, valid_data_lst, valid_peformance_dict, lr, threshold, seq_len, N_ep
):
    ...
    res_file.write("seq_len=%s " % (seq_len[ep]))
    print("Sequence length = %s " % (seq_len[ep]))
    print("-----")
...
def change_lr_seq_len_cfg(cfg_file, lr, seq_len, ep):
    ...
    config.set('batches', 'max_seq_length_train', str(seq_len[ep]))
